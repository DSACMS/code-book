...
def test_extract_file_metadata_valid():
    """
    Verify regex correctly parses year and season
    from standard naming conventions.
    """
    year, raw_season, fmt_season = extract_file_metadata(
        "Data Files/sfpuf2023_1_fall.csv"
    )
    assert year == 2023
...
def test_extract_file_metadata_invalid():
    ...
    with pytest.raises(
        ValueError,
        match=re.escape("Could not extract year and season from the file name format."),
    ):
...
def test_build_codebook_data_valid(mock_read_data, mock_read_excel, mock_parse_catalog):
    ...
    assert any(
        d["code"] == 1 and d["label"] == "Yes" and d["frequency"] == 3
        for d in yesno_dist
    )
    assert any(
        d["code"] == 2 and d["label"] == "No" and d["frequency"] == 1
        for d in yesno_dist
    )
    assert any(
        d["code"] == "." and d["label"] == "Missing" and d["frequency"] == 1
        for d in yesno_dist
    )
    ...
    assert any(
        d["code"] == "." and d["label"] == "Missing" and d["frequency"] == 2
        for d in age_dist
    )
    assert any(
        d["code"] == "R" and d["label"] == "Refused" and d["frequency"] == 1
        for d in age_dist
    )
    assert any(
        d["code"] == "D" and d["label"] == "Don't know" and d["frequency"] == 1
        for d in age_dist
    )
    assert any(
        d["code"] == "LOW-HIGH"
        and d["label"] == "Range of Values"
        and d["frequency"] == 1
        for d in age_dist
    )
